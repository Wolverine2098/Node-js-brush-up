module.exports = {
    forSearch: function (srchResp) {
        ...
        function getCityData(srchResp,cityName){
            ...
            let flname = srchResp['guess']['guess']['final_query_string'] ? srchResp['guess']['guess']['final_query_string'] : '';
            let data = {
                cities : [],
        ...
        function getBizData(srchResp,cityName){
            ...
            let  queryDataType = srchResp['guess']['guess']['queryDataType'] ? srchResp['guess']['guess']['queryDataType'] : '';
            let flname = srchResp['guess']['guess']['final_query_string'] ? srchResp['guess']['guess']['final_query_string'] : '';
        ...
        try {
            let Qtype = srchResp['guess']['guess']['query_type'] ? srchResp['guess']['guess']['query_type'] : '';
            let cdMiscAppend = cdMisAppend(srchResp);
            let cityName =  srchResp['guess']['guess']['city'] &&  srchResp['guess']['guess']['city'][0] !=="all" ? srchResp['guess']['guess']['city'][0] : '';        
            ...
            let sResponse = {
                ...
                results: srchResp['results'] && srchResp['results'].length > 0 ? fieldsRemover(srchResp['results'], ['prod_multiple_photos', 'photo', 'debug_info', 'bllocpref', 'isq', 'fcpurl', 'lastactiondatet', 'itemcurrency', 'zoomed_image', 'phone',
                ...
                adultDetected: srchResp['guess']['guess']['adultDetected'] ? srchResp['guess']['guess']['adultDetected'] : '',
                query_type: Qtype,
                live_mcats: brandsAndCategories.categories ? brandsAndCategories.categories : '',
                catid_of_topmost_mcat: srchResp['guess']['guess']['catid_of_topmost_mcat'] ? srchResp['guess']['guess']['catid_of_topmost_mcat'] : '',
                topmost_mcat: srchResp['guess']['guess']['topmost_mcat'] ? fieldsRemover(srchResp['guess']['guess']['topmost_mcat'], ['breadcrumb', 'duplicate_mcat_info', 'catid', 'catname', 'frequency', 'groupid', 'groupname', 'isgeneric', 'islive', 'mcat_accuracy_level', 'source'], true) : '',
                queryDataType: srchResp['guess']['guess']['queryDataType'] ? srchResp['guess']['guess']['queryDataType'] : '',
                final_query_string: srchResp['guess']['guess']['final_query_string'] ? srchResp['guess']['guess']['final_query_string'] : '',
                ...
                city: Qtype == 'BannedKeyword' || (srchResp['error'] && srchResp['error'] == 'OnlyLocationInQuery') ? srchResp['guess']['guess']['city'] : srchResp['facet_fields']['city'] ? srchResp['facet_fields']['city'] : '',
                ...
                contextInfo: srchResp['guess']['intent_info'] && srchResp['guess']['intent_info']['contextSearch'] ? srchResp['guess']['intent_info']['contextSearch'] : '' 
            }
            if (srchResp['error']) {
    ...
    forBuyleadList: function (req, res, body, error = false) {
        if (error) { res.status(503).send({ status: 'failed' }); }
        else {
            let responseData = JSON.parse(body);
            if (responseData && responseData.CODE == 200) {
                let bls = [];
                let list = responseData.DisplayList;
                if (list && list.length) {
                    let blsList = {};
                    if (list.length > 0) {
                        for (let i in list) {
                    ...
                    blsList['TotalBuylead'] = responseData.Allcount;
    ...
    forEnquiryBlGetISQ:function(req, res, body, error = false){
        ...
        function getISQQuestionForm(quesDetail){
            ...
            let qOptions=[];
            let optionIds=quesDetail.IM_SPEC_OPTIONS_ID ? quesDetail.IM_SPEC_OPTIONS_ID.split("##") : '';
            let optionsDesc=quesDetail.IM_SPEC_OPTIONS_DESC ? quesDetail.IM_SPEC_OPTIONS_DESC.split("##"): '';
            let isqPriority=quesDetail.IM_SPEC_OPT_PRIORITY ? quesDetail.IM_SPEC_OPT_PRIORITY.split("##"): '';                           
