...
function preloadedChunks() {
   ...
   <link rel="preload" href="${mnMinbase + '/pwagifs/ChangePassword.pwa' + chunkVersion["jsChunks"] + '.js'}" as="script" >
   <link rel="preload" href="${mnMinbase + '/pwagifs/ChangePassword.pwa' + chunkVersion["cssChunks"] + '.css'}" as="style">
...
function ChangePasswordShellStruct() {
   let crumbsObj = [
      {
...
function getChangePasswordPage(req, res, shellCallBck) {
   ChangePasswordShellStruct();
   shellCallBck(res, appShell);
}


module.exports = getChangePasswordPage;